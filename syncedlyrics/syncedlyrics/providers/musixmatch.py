...
from ..utils import Lyrics, format_time, get_cache_path
...
class Musixmatch(LRCProvider):
    ...
    def _candidates(self, query: str) -> List[dict]:
        r = self._get(
            "track.search",
            [("q", query), ("page_size", "5"), ("page", "1")],
        )
        status_code = r.json()["message"]["header"]["status_code"]
        if status_code != 200:
            self.logger.warning(f"Got status code {status_code} for {query}")
            return []
        body = r.json()["message"]["body"]
        ...
        for t in body.get("track_list", []):
            tr = t["track"]
            out.append({"id": tr["track_id"], "title": tr.get("track_name", ""),
                        "artist": tr.get("artist_name", "")})
        return out
