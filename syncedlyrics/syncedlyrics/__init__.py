...
def search(
    ...
    - `artist`: The artist name, used to disambiguate same-title songs. Only consumed by
      providers whose `get_lrc` accepts an `artist` argument (e.g. Kugeci).
    """
    ...
    for provider in _select_providers(_providers, providers):
        ...
        try:
            lrc.update(_provider_get_lrc(provider, search_term, artist))
...
def _provider_get_lrc(provider, search_term, artist):
    """Call ``provider.get_lrc``, forwarding ``artist`` only to providers whose
    signature accepts it (so artist-unaware providers keep working unchanged)."""
    if artist:
        try:
            if "artist" in inspect.signature(provider.get_lrc).parameters:
                return provider.get_lrc(search_term, artist=artist)
        except (TypeError, ValueError):
            pass
    return provider.get_lrc(search_term)
